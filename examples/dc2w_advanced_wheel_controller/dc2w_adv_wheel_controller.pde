...
double turnRadius = 12.2; //Bend radius is the distance for one wheel to another, it is the shaft length, in centimeters
...
Continuous2Wheels c2w(&rightWheel, &leftWheel, wheelRadius, turnRadius, resistance);

void setup(){
  rightWheel.attach(rightWheelPin);
  leftWheel.attach(leftWheelPin);
  // c2w.setDebugMode(true); //setting the debug mode for true to show real time logs
  // c2w.setTurnSmooth(0.2); //Applying a smooth to bends
}
